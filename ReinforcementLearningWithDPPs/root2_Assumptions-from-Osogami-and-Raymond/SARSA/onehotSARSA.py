...
class onehotSARSA:
    def __init__(self, env, α=0.5, γ=1, ε=0.0, model=1):
        self.env = env
        self.α = α
        self.γ = γ
        self.Q = np.ones(28) + np.random.uniform(-0.01, 0.01, 28)
        self.ε = ε
        ...
        if model==1:
            self.q = self.q_m1
            self.updateQ = self.updateQ_m1
        if model==2:
            self.q = self.q_m2
            self.updateQ = self.updateQ_m2
    ...
    def updateQ_m1(self, state, action, update):
        x = self.φ(state, action)
        for i in x: self.Q[i] += update# grad==1

    def q_m2(self, s, a):
        x = self.φ(s, a)
        return np.sum(np.log(self.Q[x]**2))

    def updateQ_m2(self, state, action, update):
        x = self.φ(state, action)
        for i in x: self.Q[i] += update/self.Q[i]# grad propto 1/Qi
...
if __name__=="__main__":
    ...
    env = BlockerTask()
    agent = onehotSARSA( env, α=0.2, γ=0.9, ε=0.1, model=2 )
    rews = agent.run( mxsteps=350000 )
    [x, y] = list(zip(*rews))
    plt.ylim([-1, -0.6])
    plt.plot(x, y)
    plt.show()
